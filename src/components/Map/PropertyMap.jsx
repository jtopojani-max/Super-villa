...
const COPY = {
  sq: {
    title: "Vendndodhja",
    approx: "Lokacion i perafert",
    loading: "Duke ngarkuar harten...",
    directions: "Merr drejtimin",
    openMaps: "Hap ne Maps",
  },
  en: {
    title: "Location",
...
function getFallbackCoords(location) {
  ...
  const key = Object.keys(CITY_COORDS).find((city) => normalizedLocation.includes(normalize(city)));
    ...
    <div className="property-map">
      <div className="property-map__header">
        <div>
          <h3 className="property-map__title">
            <span>Map</span> {copy.title}
          </h3>
          {displayAddress && <p className="property-map__address">{displayAddress}</p>}
        </div>
        {isApproximate && <span className="property-map__approx-badge">{copy.approx}</span>}
      </div>
      ...
      <div className="property-map__container">
        {!isLoaded ? (
          ...
          >
            {!isApproximate && mapReady && <Marker position={center} title={title} icon={markerIcon} />}
          </GoogleMap>
        )}
      </div>
      ...
      <div className="property-map__footer">
        <div className="property-map__footer-actions">
          ...
          <a href={googleMapsUrl} target="_blank" rel="noopener noreferrer" className="property-map__gmaps-btn">
            {copy.openMaps}
          </a>
        </div>
      </div>
